import json
from pathlib import Path
from dataclasses import dataclass
from typing import Optional

@dataclass
class Sample:
    image_path: str
    ground_truth: str
    sample_id: str = ""

def load_dataset(dataset_dir: str) -> list[Sample]:
    dataset_dir = Path(dataset_dir)
    if not dataset_dir.exists():
        raise FileNotFoundError(f"Dataset directory not found: {dataset_dir}")

    samples = []
    image_exts = {".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".tif", ".webp"}

    # ── Cấu trúc B: labels.json hoặc labels.txt ──
    for label_file in ["labels.json", "labels.txt", "gt.txt", "annotation.txt"]:
        label_path = dataset_dir / label_file
        if label_path.exists():
            samples = _load_from_label_file(label_path, dataset_dir, image_exts)
            if samples:
                print(f"[Dataset] Loaded {len(samples)} samples from '{label_file}'")
                return samples

    # ── Cấu trúc A & C: duyệt đệ quy tìm image + txt cùng tên ──
    for img_path in sorted(dataset_dir.rglob("*")):
        if img_path.suffix.lower() not in image_exts:
            continue
        txt_path = img_path.with_suffix(".txt")
        if txt_path.exists():
            gt = txt_path.read_text(encoding="utf-8").strip()
            sample_id = str(img_path.relative_to(dataset_dir))
            samples.append(Sample(str(img_path), gt, sample_id))

    if not samples:
        raise ValueError(
            f"Không tìm thấy dữ liệu trong '{dataset_dir}'.\n"
            "Xem README.md để biết cách chuẩn bị dataset."
        )

    print(f"[Dataset] Loaded {len(samples)} samples (image+txt pairs)")
    return samples


def _load_from_label_file(label_path: Path, root: Path, image_exts: set) -> list[Sample]:
    samples = []

    if label_path.suffix == ".json":
        with open(label_path, encoding="utf-8") as f:
            data = json.load(f)
        # Hỗ trợ dict {"filename": "gt"} hoặc list [{"file":..., "text":...}]
        if isinstance(data, dict):
            items = data.items()
        elif isinstance(data, list):
            items = [(d.get("file", d.get("filename", "")), d.get("text", d.get("label", "")))
                     for d in data]
        else:
            return []
        for fname, gt in items:
            img_path = _find_image(root, fname, image_exts)
            if img_path:
                samples.append(Sample(str(img_path), str(gt), fname))

    else:  # .txt với các định dạng phổ biến
        with open(label_path, encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                # tab-separated: "filename\tground truth"
                if "\t" in line:
                    parts = line.split("\t", 1)
                elif "," in line and not line.startswith("/"):
                    parts = line.split(",", 1)  # csv
                else:
                    continue
                if len(parts) == 2:
                    fname, gt = parts
                    img_path = _find_image(root, fname.strip(), image_exts)
                    if img_path:
                        samples.append(Sample(str(img_path), gt.strip(), fname.strip()))
    return samples


def _find_image(root: Path, fname: str, image_exts: set) -> Optional[Path]:
    """Tìm file ảnh theo tên, thử nhiều đường dẫn."""
    candidates = [
        root / fname,
        root / "images" / fname,
        root / Path(fname).name,
    ]
    # Nếu không có extension, thử các ext phổ biến
    if not Path(fname).suffix:
        for ext in image_exts:
            candidates.append(root / (fname + ext))
            candidates.append(root / "images" / (fname + ext))
    for p in candidates:
        if p.exists():
            return p
    return None
